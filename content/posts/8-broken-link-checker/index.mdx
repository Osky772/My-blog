...
Ostatnio w pracy miałem okazję skorzystać z tej biblioteki. Zaimplementowałem sprawdzanie wszystkich linków w dokumentacji naszego produktu. Ostatecznie napisany skrypt jest bardzo prosty. Najpierw dokumentacja jest budowana, następnie uruchamiany jest <ExternalPostLink name="http-server" url="https://github.com/http-party/http-server"/> w folderze z bundlem aplikacji, i w końcu uruchamiane jest skanowanie linków na tej stronie.
