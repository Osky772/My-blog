...
W zasadzie wystarczająca może być informacja, że link jest zesputy, ale chcemy jeszcze przefiltorować informację sprawdzając `statusCode`. Status `200` nie powinien nigdy się tutaj pojawić, ale `undefined` już może. Dodatkowo wykluczymy sobie status `429` o którym już wspominałem. 

Dodatkowo dla zesputych linków wewnętrzych chcemy użyć `console.error`, a dla zewnętrzych `console.warn`, aby zaznaczyć, że najważniejsze dla nas są działające linki wewnętrzne. 
